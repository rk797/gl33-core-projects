...
void _log(const S1 &prefix, const S2 &c, const S3 &format_str, Args &&...args){
    ...
    auto white_message
        = fmt::format(fg(fmt::color::white), fmt::runtime(format_str), std::forward<Args>(args)...);
...
void debug(const S &format_str, Args &&...args){
    _log("DEBUG", fmt::color::purple, format_str, args...);
}

template<typename S, typename... Args>
void info(const S &format_str, Args &&...args){
    _log("INFO", fmt::color::cyan, format_str, args...);
}
