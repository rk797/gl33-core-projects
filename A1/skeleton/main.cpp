...
FractalControl* pFractalControl = new FractalControl();
...
void TreeRecursion_Branch(CPU_Geometry& _CPUGeom, glm::vec3 _Start, glm::vec3 _End, int _D, bool bLastRotateDirection, int _CurrentIteration)
{
	...
	else
	{
		...
		int NextIteration = _CurrentIteration + 1;
		AddLine(_End, q1);
		TreeRecursion_Branch(_CPUGeom, q1, q2, NextDepth, bLastRotateDirection, NextIteration);
		TreeRecursion_Branch(_CPUGeom, _End, q3, NextDepth, !bLastRotateDirection, NextIteration);
	}
...
int main() {
	...
	{ // Extra scope to ensure all GL releated destructors are called (for example in Window class) before glfwTerminate
		...
		pFractalControl->CurrentDepth = 0; // Start at iteration 0

		// CALLBACKS
		std::shared_ptr<FractalCallbacks> callback_ptr = std::make_shared<FractalCallbacks>(shader, *pFractalControl);
		...
		while (!window.shouldClose()) 
		{
			...
			{
				ImGui::Begin("Fractal Control");

				// depth Slider
				const int MaxDepth = pFractalControl->CurrentScene == SceneType::TREE ? 11 : 8;
				if (ImGui::SliderInt("Recursion Depth", &pFractalControl->CurrentDepth, 0, MaxDepth))
				{
					pFractalControl->bNeedsUpdate = true;
				}

				// scene Selection
				const char* Items[] = { "Sierpinski", "Anti-Snowflake", "Tree" };
				int CurrentItem = (int)pFractalControl->CurrentScene;
				if (ImGui::Combo("Scene Type", &CurrentItem, Items, IM_ARRAYSIZE(Items)))
				{
					pFractalControl->CurrentScene = (SceneType)CurrentItem;
					pFractalControl->bNeedsUpdate = true;
				}
			...
			if (pFractalControl->bNeedsUpdate)
			{
				
				cpuGeom.verts.clear();
				cpuGeom.cols.clear();

				if (pFractalControl->CurrentScene == SceneType::SIERPINSKY)
				{
					SierpinskiRecursion_SubTriangle(cpuGeom, mainTriangle, pFractalControl->CurrentDepth);
				}
				else if (pFractalControl->CurrentScene == SceneType::ANTI_SNOWFLAKE)
				{
					glm::vec3 blueCol(0.0f, 0.0f, 1.0f);
					glm::vec3 redCol(1.0f, 0.0f, 0.0f);
					glm::vec3 greenCol(0.0f, 1.0f, 0.0f);
					
					KockRecursion_AntiSnowflake(cpuGeom, v0, v1, blueCol, redCol, pFractalControl->CurrentDepth);
					KockRecursion_AntiSnowflake(cpuGeom, v1, v2, redCol, greenCol, pFractalControl->CurrentDepth);
					KockRecursion_AntiSnowflake(cpuGeom, v2, v0, greenCol, blueCol, pFractalControl->CurrentDepth);
				}

				else if (pFractalControl->CurrentScene == SceneType::TREE)
				{
					TreeRecursion_Branch(cpuGeom, t1, t2, pFractalControl->CurrentDepth, 1, 0);
				}

				gpuGeom.setVerts(cpuGeom.verts);
				gpuGeom.setCols(cpuGeom.cols);

				pFractalControl->bNeedsUpdate = false;
			}
